...
model_path="./saved_model/seq128_b64_class_changefalse_nlu_cosent_like_uda_0_simcse_cls_2.pt"
...
def dialogue_pridect(path,model,primary_data,primary_vec,thres,similar_data,predict_save_path):
    ...
    with open(predict_save_path,"w",encoding="utf-8") as f:
        for i,raw in enumerate(context_data):
            ...
            if isinstance(context_id[i],str):
                ...
                if "_begin" not in raw and "None" not in raw:
                    # if "http" not in raw or "<img" not in raw:
                    idata=re.sub("[http|https]://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+","",raw)
                    idata=re.sub("(<img).*(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+","<img",idata)
            ...
            if len(dialogue_input_data)>0:
                ...
                if sop_cos_max>thres:
                    ...
                    # print(primary_data[sop_cos_argmax],":",sop_cos_max.cpu(),":",dialogue_input_data[-1],":",similar_data[sop_cos_argmax])
                else:
                    context_pridect.append(None)
                    context_pridect_score.append(None)
            else:
...
def read_true_sop(true_path):
    ...
    s_true=[(context_id[i],context_sop_name[i]) for i,raw in enumerate(context_label) if raw=="是"]
    s_false=[(context_id[i],None) for i,raw in enumerate(context_label) if raw=="否"]
    ...
    # for raw in nlu_predict_false:
    #     if raw[0] not in sop_nlu_predict:
    #         sop_nlu_predict[raw[0]]=[raw[1]]
    #     else:
    #         sop_nlu_predict[raw[0]].extend([raw[1]])
    return sop_true,sop_nlu_predict
...
if __name__ == '__main__':
    ...
    sop_predict=dialogue_pridect("./sop_test_0825_data.xlsx",model,primary_data,primary_vec,threshold,similar_data,predict_save_path)

    sop_true,sop_nlu_predict=read_true_sop("推荐SOP数据804-811-手动进入.xlsx")
